...
function findSlatWidth(platformLength, startSlatWidth = 35, endSlatWidth = 45, stepSlatWidth = 0.1) {
  let validSlatWidths = [];
  let validNumSlats = [];
  for (let slatWidth = startSlatWidth; slatWidth <= endSlatWidth; slatWidth += stepSlatWidth) {
...
function adjustPlatforms(platformLengths) {
  ...
  for (let i = 0; i < platformLengths.length; i++) {
    ...
    for (let adjustedLength of range(platformLength - slatWidthPlusOrMinus, platformLength + slatWidthPlusOrMinus + slatStepSize, slatStepSize)) {
      ...
      if (slatWidths.length > 0) {
        for (let j = 0; j < slatWidths.length; j++) {
          ...
          let otherPlatforms = platformLengths.slice(0, i).concat(platformLengths.slice(i + 1));
          let otherWidthsAndSlats = otherPlatforms.map(platLen => findSlatWidth(platLen, width - slatWidthPlusOrMinus, width + slatWidthPlusOrMinus, slatStepSize));
...
for (let i of range(platformLengths[0] - rangeI, platformLengths[0] + rangeI)) {
  for (let j of range(platformLengths[1] - rangeJ, platformLengths[1] + rangeJ)) {
    for (let k of range(platformLengths[2], platformLengths[2] + rangeK)) {
      matrixPlatforms.push([i, j, k]);
    }
  }
}
//console.table(matrixPlatforms);
...
for (let platformLengths of matrixPlatforms) {
  ...
  for (let i = 0; i < platformLengths.length; i++) {
    ...
    let filteredResults = results[results.length - 1].filter(result => result[0] === platformLength);
    if (filteredResults.length > 0) {
      best = filteredResults.reduce((prev, curr) => Math.abs(curr[0] - platformLength) < Math.abs(prev[0] - platformLength) ? curr : prev);
    }
